...
public class DoorbellSetActivity extends BaseActivity {
    ...
    @Override
    public void init() {
        HttpAction.getHttpAction(this).getDoorbellConfig(IMEI, new IDataListener<DoorbellConfig>() {
            @Override
            public void onSuccess(DoorbellConfig doorbellConfig) {
...
//                    mDoorbellParam = new DoorbellParam();
    ...
    private void initView() {
        if (mDoorbellConfig == null)
            return;
//        cbNetPush.setChecked(mDoorbellConfig.get() == 1);
